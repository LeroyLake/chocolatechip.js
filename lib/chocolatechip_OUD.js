...
global.ChocolateChip = {
  ...
  _componentDidMount() {
    ...
    // Hide consent if user navigated the website before
    if (localStorage.getItem('initialUrl') != null 
      && localStorage.getItem('initialUrl') != window.location.href
      && this._state.isChocolateChipHidden !== true) {
        acceptAll();
    };
  ...
  _bindEvents() {
    ...
    body.addEventListener('click', () => {
      ...
      console.log('body click', bodyClickCount);
      bodyClickCount += 1;
      ...
      // assuming body is clicked once by the 'website' itself
      if (this._state.isChocolateChipHidden !== true && hasBeenClicked === true && bodyClickCount > 1) {
        acceptAll();
      }
